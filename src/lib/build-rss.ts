...
import { textBlock } from './Docs/renderers'
import getBlogIndex from './Docs/getBlogIndex'
import getDocsUsers from './Docs/getDocsUsers'
...
function createRSS(blogPosts = []) {
  ...
  <feed xmlns="http://www.w3.org/2005/Atom">
    ...
    <link href="/" />
    <updated>${NOW}</updated>
    <id>My Docs Blog</id>${postsString}
  </feed>`
}
...
async function main() {
  ...
  const { users } = await getDocsUsers([...neededAuthors])
